...
export default ({ storeName }) => {
    ...
    const [responsive, setResponsive] = useState(!(window.innerWidth < 1400))
            ...
            ;(async () => {
                const res = await Axios.post(
                        ...
                        'http://localhost:8081/graphql',
                    {
                        query: `
                    {
                        verifyToken(token:"${localStorage.getItem(
                            'authorization'
                        )}" storeName:"${storeName}") {
                            admin
                        }
    ...
    window.addEventListener('resize', () =>
        window.innerWidth < 1400 ? setResponsive(false) : setResponsive(true)
    )
            ...
            {!!storeInfo && (
                <>
                    ...
                    <Link
                        ...
                        className="logo"
                    >
                        <img alt={storeInfo.insta} src={storeInfo.logoLink} />
                    </Link>
                    ...
                    {responsive && (
                        <footer>
                            <a href="https://twitter.com/bugextreme1">
                                Desenvolvido por Robert Damaceno
                            </a>
                        </footer>
                    )}
                </>
